...
import tech.bluchain.sample.examplecorda.contract.PersonContract
...
* run vaultQuery contractStateType: tech.bluchain.sample.examplecorda.state.PersonState
*
* */
...
object CreatePerson {
    ...
    class CreatePersonFlow(
        ...
        override fun call(): SignedTransaction {
            ...
            val indexDocument = PersonSchemaV1.PersistentPerson::document.equal(document)
            val criteria = QueryCriteria.VaultCustomQueryCriteria(expression = indexDocument)
            val personList = serviceHub.vaultService.queryBy<PersonState>(criteria).states
            ...
            val person = PersonModel(ourIdentity, Instant.now(), name, document)

            // criando um estado de pessoa
            val personState = PersonState(person)

            // criando o command para validação do contract
            val txCommand = Command(
                PersonContract.Commands.CreatePerson(),
                personState.participants.map { it.owningKey }
            )
            ...
            val txBuilder = TransactionBuilder(notary)
                ...
                .addOutputState(personState, PersonContract::class.java.canonicalName)
    ...
    * */
    @InitiatedBy(CreatePersonFlow::class)
